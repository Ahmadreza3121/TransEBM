! Output the simulated results into netcdf
      ...
      subroutine monthly_output(filename_base, yrs, yr_loop_start, config)
...
!     Albedo field.
      character*(*) a_name
      parameter (a_name='albedo')
      integer a_varid

!     Geography/ world map field.
      character*(*) m_name
      parameter (m_name='map')
      integer m_varid
...
!      parameter (rec_units = 'months since 1950-01-16 12:00:00')
      ...
      integer m_out(NLONS,NLATS,NLVS,NRECS), m(NLONS,NLATS)
...
!     Error handling.
      ...
      tmp = wrk_dir // filename_base // '/' // filename_base // '_monthly-output.nc'
...
!     write heat capacity output to netcdf file 
      ...
      period_cnt = 1
      yr_cnt = 0
      yr_loop: do yr = yr_loop_start, yrs
...
!            S_out(:,:,NLVS,:) = S(:,:,:)
            ...
            !            BINfile = trim(adjustl(tmp))
                        BINfile = wrk_dir//filename_base//'/bin/T/T_yr-'// &
